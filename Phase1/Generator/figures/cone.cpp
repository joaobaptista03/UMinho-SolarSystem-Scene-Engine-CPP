...
void generateCone(const std::string& fileName, float radius, float height, int slices, int stacks){
    ...
    outFile << "0," << height << ",0" << "," << std::endl; // Tip of the cone is now the first vertex
    ...
    for (int stack = 1; stack < stacks; ++stack) { // Start from 1 to skip the base center and apex
    ...
    outFile << "0," << "0" << ",0" << "," << std::endl; // Tip of the cone is now the first vertex
    ...
    for (int stack = 0; stack < stacks ; ++stack) {
        for (int slice = 0; slice < slices; ++slice) {
            ...
            if (stack == 2) {  // If it's the first stack, connect to the apex
                outFile << apexIndex << "," << bottomLeft << "," << bottomRight <<  "," <<std::endl;
            } else {  // For stacks between the apex and base
